...
void SceneIO::AppendColliders(QDomElement root, Scene* scene)
{
	...
	for (SceneNodeIterator iter = scene->Begin(); iter.IsValid(); ++iter)
	{
		if ((*iter)->HasRenderable() == true && (*iter)->GetType() == SceneNode::Type::SCENE_COLLIDER)
		{
			QDomElement cNode = m_document.createElement("Collider");
			SceneCollider* sCollider = dynamic_cast<SceneCollider*>((*iter)->GetRenderable());
			ImplicitCollider iCollider(*sCollider->GetImplicitCollider());

			iCollider.ApplyTransformation((*iter)->GetCTM());
			if (IsEqual(sCollider->GetVelocityMagnitude(), 0.0f) == false)
			{
				iCollider.velocity = sCollider->GetVelocityMagnitude() * sCollider->GetWorldVelocity((*iter)->GetCTM());
			}
			else
			{
				iCollider.velocity = Vector3(0, 0, 0);
			}

			cNode.setAttribute("type", static_cast<int>(iCollider.type));
...
void SceneIO::AppendVector(QDomElement node, const QString name, const Vector3 v)
{
